...
typedef struct {
    uint16_t           last_valid_cm;  // Última leitura válida para heartbeat
} sensor_state_t;
...
static void sensor_tick(uint8_t idx) {
    ...
    if (reading.valid) {
        ...
        g_sensor[idx].last_valid_cm = reading.distance_cm;
    } else {
